...
export default async function Home() {
  ...
  return (
    <div>
      <section className="relative h-[560px] flex items-end bg-ink">
        <img src="https://images.unsplash.com/photo-1552674605-db6ffd4facb5?w=1600&q=80" alt="Marathon runner" className="absolute inset-0 w-full h-full object-cover opacity-60" />
        ...
        <div className="relative max-w-6xl mx-auto px-5 pb-16 w-full text-white">
          <span className="bib text-white border-white">№001</span>
          <h1 className="font-black text-4xl md:text-6xl leading-[0.95] mt-4 mb-4 tracking-tight">
            EVERY FINISH<br/>LINE STARTS<br/>WITH A BIB.
          </h1>
          <p className="max-w-md text-white/75 mb-7">Apex Athletics organizes marathons, running, fitness and adventure events. Register, train, and cross your next finish line with us.</p>
          <div className="flex flex-wrap gap-3">
            <Link href="/events" className="btn btn-primary">Explore Events <ArrowRight size={15}/></Link>
            <Link href="/events" className="btn btn-outline !border-white !text-white">Register Now</Link>
          </div>
        </div>
      </section>
      ...
      <section className="py-16" style={{ background: "#F2EFE9" }}>
        <div className="max-w-6xl mx-auto px-5 grid md:grid-cols-3 gap-8">
          {[
            ...
            <div key={i} className="bg-white p-7 rounded-sm border" style={{ borderColor: "#E7E2D9" }}>
              <div className="w-11 h-11 rounded-sm flex items-center justify-center mb-4" style={{ background: "#FFF3EA", color: "#C43D0E" }}>{f.icon}</div>
      ...
      {products?.length > 0 && (
        <section className="max-w-6xl mx-auto px-5 py-16">
          <div className="flex items-center justify-between mb-8">
            ...
            <Link href="/store" className="hidden md:flex items-center gap-1 text-sm font-bold text-accentDark">View all <ArrowRight size={16}/></Link>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-5">
            {products.map((p) => (
              <div key={p.id} className="border rounded-sm overflow-hidden bg-white" style={{ borderColor: "#E7E2D9" }}>
        ...
        <section className="py-16 bg-ink">
          <div className="max-w-6xl mx-auto px-5">
            ...
            <div className="flex flex-wrap gap-x-10 gap-y-4">
              {sponsors.map((s) => <span key={s.id} className="text-white/80 font-bold text-lg tracking-tight">{s.name}</span>)}
            </div>
          </div>
        </section>
      )}
...
function EventCard({ e }) {
  return (
    <Link href={`/events/${e.slug}`} className="group border rounded-sm overflow-hidden bg-white block" style={{ borderColor: "#E7E2D9" }}>
      <div className="relative h-52 overflow-hidden">
        {e.banner_url && <img src={e.banner_url} alt={e.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />}
      </div>
      <div className="p-5">
